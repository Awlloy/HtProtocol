...
int readMessage(void *buf,int size,HtProtocolContext *context,int time_out){
    ...
    while(get_passtime(&timestamp)<time_out){
        ...
        printf("window_id_offest %d %d  recover_buf.size %d\n",window_id_offest,context->read_fifo.size,recover_buf.size);
        // printf("head get %d\n",context->read_check[context->read_fifo.head]);
        ret=-1;
        printf("head number %d\n",context->read_fifo.fifo[context->read_fifo.head].number);
        ...
        // }else if(!context->read_check[window_id]){//未缓存该帧
            ...
            if(!context->read_check[window_id]){//未缓存
                //缓存该帧
                if(is_head){
                    ret=replace_queue_from_user(&(context->read_fifo),context->read_check,true,window_id_offest,recover_buf.buf+3,recover_buf.size-4,recover_buf.number);
                }else{//
                    ret=replace_queue_from_user(&(context->read_fifo),context->read_check,true,window_id_offest,recover_buf.buf+2,recover_buf.size-3,recover_buf.number);
                }
            }else{
                ret=0;//已有缓存
            }
        ...
        // printf("context->read_fifo.size %d\n",context->read_fifo.size);
